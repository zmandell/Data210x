...
for file in range(50):
    filpath = '/Users/zach.mandell/Downloads/free-spoken-digit-dataset-master/recordings/0_jackson_'+str(file)+'.wav'
    zero.append(wavfile.read(filpath)[1])
...
zero.dropna(axis=1,how='any',inplace=True)
...
X_test = test[0:int(test.shape[0]*.25)]
...
y_test = test[int(test.shape[0]*.25):]
...
#
# .. your code here ..
X_train = train[:,0:int(test.shape[0]*.25)]
y_train = train[:,int(test.shape[0]*.25):]
